...
public class NonRegisterHCEFragment extends Fragment {
    ListView listView;
    ArrayList<HashMap<String, String>> indtabresult;
    ArrayList<HashMap<String, String>> nonRegisteredHCElist;
    int count=1;
    String NonRegisterHCEID;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        NonRegisterHCEID= args.getString("NonRegisterHCEID");
        email= args.getString("email");
        password= args.getString("password");
        username= args.getString("username");
        isEdit= args.getString("isEdit");
        nonRegisteredHCElist=new ArrayList<HashMap<String, String>>();
        if(indtabresult!=null) {
            for (int i = 0; i < indtabresult.size(); i++) {

                if (indtabresult.get(i).get("ActionType").equals(NonRegisterHCEID)) {
                    ...
                    nonRegisteredHCElist.add(map);
                    count++;
                }
            }
        }
        NotSealedAdapter notSealedAdapter=new NotSealedAdapter(getContext(),nonRegisteredHCElist,NonRegisterHCEID,email,password,username,isEdit);
